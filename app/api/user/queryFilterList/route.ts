...
export async function POST(req: NextRequest) {
  ...
  if (Object.keys(filterInfo).length === 0) {
    ...
    // console.log(
    //     filterInfo['currentAddress'].filter((item) => item !== "全部").join(',')
    // );
    for (const key in filterInfo) {
      // 用contains判读来获取 例如 上海-全部 下面的所有人群，不能用equals
      if (key === "currentAddress" || key === "originalAddress" || key === 'customTags') {
        filterWhere[key] = {
          array_contains: filterInfo[key].filter((item) => item !== "全部")
        };
      } else {
        filterWhere[key] = {
